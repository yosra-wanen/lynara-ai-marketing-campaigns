    ...
    <div className="p-8 max-w-2xl mx-auto">
      ...
      <div className="flex gap-2 mb-6">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'manual' ? 'bg-[#E1306C] text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
          ✍️ Création manuelle
        </button>
        <button
          onClick={() => setActiveTab('csv')}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'csv' ? 'bg-[#E1306C] text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
          📁 Importer CSV
        </button>
      </div>
      ...
      {activeTab === 'csv' && (
        <div className="bg-white rounded-lg border p-6 space-y-4">
          ...
          <form onSubmit={handleCsvImport} className="space-y-4">
            <div className={`border-2 border-dashed rounded-lg p-8 text-center transition-colors ${csvFile ? 'border-[#E1306C] bg-[#E1306C]/5' : 'border-gray-300'}`}>
              {csvFile ? (
                <div>
                  <p className="text-[#E1306C] font-medium">📁 {csvFile.name}</p>
          ...
          {importResult && (
            <div className={`rounded-lg p-4 ${importResult.imported > 0 ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
              ...
              <a href="/leads" className="mt-2 inline-block text-sm text-[#E1306C] hover:underline">
