...
export default function LeadsPage() {
  ...
  async function fetchLeads() {
    try {
      ...
      const res = await fetchWithTimeout(`${API_URL}/leads/?${params}`)
      const json = await res.json().catch(() => ({}))
      if (!res.ok) {
        const msg = Array.isArray(json.detail) ? json.detail.map((e: any) => e.msg).join(', ') : (json.detail ?? res.statusText)
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <a href="/leads/nouveau" className="px-4 py-2 bg-[#7C4DFF] text-white rounded-lg hover:bg-[#6D3FEB] transition-colors">
          + Nouveau Lead
        </a>
      </div>
      ...
      <form onSubmit={handleSearch} className="flex gap-4 mb-6">
        ...
        <button type="submit" className="px-4 py-2 bg-[#7C4DFF] text-white rounded-lg hover:bg-[#6D3FEB]">
          Rechercher
        </button>
      </form>
      ...
      <p className="text-sm text-gray-500 mb-2">{loading ? 'Chargement...' : `${total} lead(s) trouvé(s)`}</p>
            ...
            {loading ? (
              ...
              <tr key={lead.lead_id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 text-sm flex gap-2">
                  <a href={`/leads/${lead.lead_id}`} className="text-[#7C4DFF] hover:text-[#6D3FEB]">Voir</a>
